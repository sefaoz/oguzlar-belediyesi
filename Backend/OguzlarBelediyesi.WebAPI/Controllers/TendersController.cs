...
public sealed class TendersController : ControllerBase
{
    ...
    public async Task<IActionResult> Create([FromForm] TenderRequest request, [FromForm] List<IFormFile>? documentFiles)
    {
        ...
        // Handle existing/metadata from JSON if provided (unlikely in create but possible)
        ...
        if (documentFiles is not null && documentFiles.Count > 0)
        {
            ...
            {
                var path = await SaveFileAsync(file, "uploads/tenders");
        ...
        var tender = new Tender
        {
            ...
            Status = request.Status ?? "Open", // Default status instead of empty
        ...
        // Repository AddAsync calls SaveChangesAsync
    ...
    [DisableRequestSizeLimit]
    [RequestFormLimits(MultipartBodyLengthLimit = 500 * 1024 * 1024)] // 500MB
    public async Task<IActionResult> Update(Guid id, [FromForm] TenderRequest request, [FromForm] List<IFormFile>? documentFiles)
    {
        ...
        // 1. Keep existing documents passed in JSON
        if (!string.IsNullOrEmpty(request.DocumentsJson))
        {
             try 
            {
